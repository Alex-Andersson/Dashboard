...
const Item = ({
  ...
  if (navItem.child) {
    return (
      <li key={navItem.name}>
        <div
          className="px-4 flex items-center justify-between py-3 cursor-pointer"
          onClick={toggleNavItem}
        >
          <div className="flex items-center">
            {navItem.icon}
            <span className="ml-2 text-md text-white">{navItem.name}</span>
          </div>
          <ChevronRightIcon
      ...
      <li key={navItem.name}>
        <NavLink
          ...
          className={({ isActive }) => {
            ...
            return `px-4 flex items-center justify-between py-3 `;
          }}
        >
          <div className="flex items-center">
            {navItem.icon}
